...
interface DecorSectionProps {
  decorType: DecorType;
  decorContractors: string;          // comma-separated IDs from API
  contractors: Contractor[];          // full list from API
  onChangeType: (val: DecorType) => void;
  onChangeContractors: (val: string) => void;  // sends back comma-separated
}
...
export default function DecorSection({
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {(["internal", "external"] as const).map((type) => (
            <button key={type} onClick={() => onChangeType(type)}
              style={{ flex: 1, padding: 10, borderRadius: "var(--radius-md)", border: decorType === type ? "2px solid var(--accent)" : "1.5px solid var(--border-default)", background: decorType === type ? "var(--accent-bg)" : "#fff", fontSize: 13, fontWeight: 600, fontFamily: "var(--font-primary)", cursor: "pointer", color: decorType === type ? "var(--accent)" : "var(--text-secondary)", textTransform: "capitalize", transition: "all var(--transition-fast)" }}>
            ...
            {contractors.map((c) => {
              ...
              return (
                <div key={c.id} onClick={() => toggleContractor(c.id)}
                  style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", borderRadius: "var(--radius-md)", border: `1.5px solid ${isSelected ? "var(--accent)" : "var(--border-default)"}`, background: isSelected ? "var(--accent-bg)" : "#fff", cursor: "pointer", transition: "all var(--transition-fast)" }}>
                  <div>
                  ...
                  <div style={{ width: 20, height: 20, borderRadius: 4, border: `2px solid ${isSelected ? "var(--accent)" : "var(--border-default)"}`, background: isSelected ? "var(--accent)" : "#fff", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 12, fontWeight: 700, transition: "all var(--transition-fast)" }}>
                    {isSelected && "✓"}
                  </div>
                </div>
              );
      ...
      {decorType === "external" && (
        <div style={{ padding: 12, background: "#fff", borderRadius: "var(--radius-md)", border: "1px solid var(--border-default)", textAlign: "center" }}>
